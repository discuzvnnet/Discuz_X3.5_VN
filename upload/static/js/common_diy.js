	...
	DIY.prototype = {
		...
		uploadSubmit : function (){
			if (document.uploadpic.attach.value.length<3) {
				alert('Vui lòng chọn hình ảnh bạn muốn tải lên');
				return false;
			}
		...
		cancel : function () {
			var flag = false;
			if (this.isChange) {
				flag = confirm(this.cancelConfirm ? this.cancelConfirm : 'Thoát sẽ không lưu các cài đặt trước đó của bạn. Bạn có chắc chắn để thoát?');
			}
